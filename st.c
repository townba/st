...
static void tsetmode(char, int, const int *, int);
...
static Color *xgetcolor(uint32_t, XRenderColor *, Color *);
...
csihandle(void)
{
	...
	case '?':
		...
		case 'l': /* RM -- Reset Mode */
			tsetmode(csiescseq.interm, 0, csiescseq.arg, csiescseq.narg);
			break;
		...
		case 'h': /* SM -- Set terminal mode */
			tsetmode(csiescseq.interm, 1, csiescseq.arg, csiescseq.narg);
			break;
...
xdrawglyphfontspecs(const XftGlyphFontSpec *specs, Glyph base, int len, int x, int y)
{
	...
	if (base.mode & ATTR_UNDERLINE) {
		XftDrawRect(xw.draw, *destfg,
				winx, winy + dc.font.ascent + 1, width, 1);
	}

	if (base.mode & ATTR_STRUCK) {
		XftDrawRect(xw.draw, *destfg,
				winx, winy + 2 * dc.font.ascent / 3, width, 1);
	}
